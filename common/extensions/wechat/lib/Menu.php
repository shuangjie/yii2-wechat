...
class Menu extends BaseClient {
    ...
    public function setMenu($menuList){
        ...
        $result = HttpClient::api($url, 'POST', $data, ['Content-Type' => 'application/json; charset=utf-8']);
        if($result['errcode'] == 0){
            return true;
        }
        $result['url'] = $url;
    ...
    public function setMenuJson($menuListJson){
        //json格式
        $data = $menuListJson;
        //创建默认菜单的请求地址
     ...
     * @return boolean|array
     */
    public function setPersonalMenuJson($menuListJson){
        //json格式
        $data = $menuListJson;
        //请求地址与创建普通菜单有区别
        //创建个性化菜单之前必须先创建默认菜单
        $url = "https://api.weixin.qq.com/cgi-bin/menu/addconditional?access_token=".$this->accessToken;
        $result = HttpClient::api($url, 'POST', $data, ['Content-Type' => 'application/json; charset=utf-8']);
        if($result['errcode'] == 0){
            return true;
        }
